...
# Implementación del Agente Q-Learning con Tabla Q Unificada
...
def state_to_index(agent_pos_idx, grid_width):
    return agent_pos_idx

# Función para convertir índice a posiciones (x, y)
def index_to_positions(agent_pos_idx, grid_width):
    return (agent_pos_idx // grid_width, agent_pos_idx % grid_width)

def train_and_get_policy(grid_size=(5,5), obstacles=[(1,1), (2,2), (3,3)],
                        num_episodes=1000, max_steps=100):
    """
    Entrena el agente Q-Learning en el entorno de policía y ladrón y devuelve la política en formato JSON.
    
    Parámetros:
        grid_size (tuple): Tamaño de la grilla (altura, anchura).
        obstacles (list): Lista de posiciones de obstáculos.
        num_episodes (int): Número de episodios de entrenamiento.
        max_steps (int): Número máximo de pasos por episodio.
    
    Retorna:
        policy_json (list): Política entrenada en formato JSON (lista de diccionarios).
    """
    # Crear el entorno
    ...
    for episode in range(num_episodes):
        ...
        police_state_idx = state_to_index(police_idx, env.grid_width)
        thief_state_idx = state_to_index(thief_idx, env.grid_width)
        ...
        while not done and step < max_steps:
            ...
            police_next_state_idx = state_to_index(police_next_idx, env.grid_width)
            ...
            police_next_idx, thief_next_idx = next_state
            thief_next_state_idx = state_to_index(thief_next_idx, env.grid_width)
        ...
        for agent_type in [0, 1]:  # 0=Policía, 1=Ladrón
            ...
            for agent_pos_idx in range(grid_size[0] * grid_size[1]):
                ...
                if agent_pos in obstacles:
                    ...
                    state_agent = state_to_index(agent_pos_idx, grid_size[1]) * 2 + agent_type
    ...
    print("Entrenamiento completado y política extraída exitosamente.")
