...
class RobotController:
    def __init__(self, maze, qr_detector, agent_type=0):
        """
        Inicializa el controlador del robot.
        
        Parámetros:
            maze (list of list of int): Mapa del laberinto.
            qr_detector (cv2.QRCodeDetector): Detector de QR de OpenCV.
...
def main():
    ...
    # Definir el tipo de agente: 0=Policía, 1=Ladrón
    # Puedes cambiar este valor según el rol que desees asignar al robot
    AGENT_TYPE = 0  # 0=Policía, 1=Ladrón
    ...
    robot = RobotController(maze, qr_detector, agent_type=AGENT_TYPE)

    # Inicializar contador para el envío de comandos
    frame_counter = 0

    # Variables para Q-learning
    # Eliminamos la inicialización de Q ya que la nueva función devuelve la política directamente
    politica = {}
        ...
        while True:
            ...
            if frame_counter % COMMAND_INTERVAL == 0 and not policy_generated:
                ...
                if (robot.current_row, robot.current_col) == (0, 0):
                    ...
                    # Entrenar al agente y obtener la política
                ...
                accion = politica.get((estado_actual, AGENT_TYPE), None)
                if accion:
                    logger.info(f"Acción según la política en {estado_actual} para Agent_Type {AGENT_TYPE}: {accion}")
                    # Ejecutar la acción
                    if accion == 'up':
                    ...
                    logger.warning(f"No hay acción definida en la política para la posición {estado_actual} y Agent_Type {AGENT_TYPE}.")
